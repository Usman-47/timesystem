...
router.post("/register/emp", function (req, res) {
  ...
  User.findById(req.body.key, function (err, admin) {
    // Find the company if it exists by the input key
    if (err) {
      res.send(
        "<h1>Invalid Key</h1><hr><a href='/signup/emp'>Go back</a></h2>"
      );
    } else if (!admin) {
      res.send(
        "<h1>Invalid Key or hire-date</h1><hr><a href='/signup/emp'>Go back</a></h2>"
      );
    } else if (hdate > Date.now()) {
      res.send(
        "<h1>Invalid Hire-Date</h1><hr><a href='/signup/emp'>Go back</a></h2>"
      );
    } else if (admin.admin == "admin") {
      // If the company exists for the key in the database
      var user = new User(); //create a new user instance
      ...
      user.save(function (err) {
        if (err) {
          console.log(err);
          res.send(
            "<h2>" + err.message + "<hr><a href='/signup/emp'>Go back</a></h2>"
          );
        } else {
          res.redirect("/login"); // After succesfull registration , redirect to the login page
        }
      ...
      res.send(
        "<h1>Invalid Key - 2</h1><hr><a href='/signup/emp'>Go back</a></h2>"
      );
    }
  });
});
  ...
  User.findById(req.params.id, function (err, user) {
    if (err) res.send(err);
      ...
      user.save(function (err) {
        if (err) {
          res.send(
            "<h2>" + err.message + "<hr><a href='/'>Go to Home Page</a></h2>"
          );
        } else if (user.admin == "admin") {
        } else {
          res.send(
            "<h2>Password changed successfully<hr><a href='/users/" +
              user._id +
              "'>Go back</a></h2>"
          );
        }
...
router.post(
  ...
  function (req, res) {
    ...
    if (req.user.admin == "admin")
      ...
      res.redirect("/admin");
    else res.redirect("/users/" + req.user._id); //else redirect to employee page
  }
);
